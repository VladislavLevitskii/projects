...
KERNEL_TEST_EXTRAS = {
    ...
    'thread/leaks': {
        'mm_implementation': 'tests/leak.c',
        'CFLAGS': [
            # Use only for debugging, not for bypassing leak tests
            '-DKERNEL_HEAP_ALLOCATOR_MINIMAL_IMPLEMENTATION=1',
        ]
    },
    'as/leaks': {
        'mm_implementation': 'tests/leak.c',
        'CFLAGS': [
            # Use only for debugging, not for bypassing leak tests
            '-DKERNEL_HEAP_ALLOCATOR_MINIMAL_IMPLEMENTATION=1',
            '-DKERNEL_FRAME_ALLOCATOR_MINIMAL_IMPLEMENTATION=1',
        ]
    },
}
...
KERNEL_MEMORY_MANAGEMENT_IMPLEMENTATION = {kernel_mm_implementation}
...
def main(argv):
    ...
    args.add_argument('--toolchain',
        default=os.getenv("NSWI200_TOOLCHAIN_DIR", None),
        dest='toolchain_dir',
        help='Toolchain directory.'
    )
    args.add_argument('--target',
        default=os.getenv("NSWI200_TOOLCHAIN_TARGET", None),
    ...
    args.add_argument('--print-paths-only',
        default=False,
        dest='print_paths_only',
        action='store_true',
        help='Do not create config.mk, only detect toolchain location.'
    )
    ...
    if config.print_paths_only:
        details = {
            'toolchain': {
                'directory': config.toolchain_dir,
                'target': config.toolchain_target,
            },
        }
    ...
    if config.kernel_test is not None:
        ...
        if config.kernel_test in KERNEL_TEST_EXTRAS:
            ...
            if 'mm_implementation' in test_extras:
                kernel_mm_implementation = test_extras['mm_implementation']
    ...
    with open('config.mk', 'w') as f:
        ...
        f.write(CONFIG_MK_FMT.format(
            ...
            kernel_mm_implementation=kernel_mm_implementation,
            userspace_test_sources=userspace_test_sources,
        ))
